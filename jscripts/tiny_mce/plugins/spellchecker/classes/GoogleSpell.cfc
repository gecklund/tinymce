...
<cfcomponent name="GoogleSpell" extends="SpellChecker" hint="Uses Google spellchecker API." output="false">
	...
	<!---
	 * Learns a word locally.
	 *
	 * @param {String} $word word to learn.
	--->
	<cffunction name="learnWord" access="public" returntype="any" output="false" hint="Learns a word">
		<cfargument name="lang" type="string" hint="Language code like sv or en." />
		<cfargument name="word" type="string" hint="Word to learn" />

		<cfset var Local = StructNew() />
		
		<cftry>
			
			<cfif FileExists( ExpandPath("localdic/local.txt") )>
				<cffile action="append" file="#ExpandPath('localdic/local.txt')#" addnewline="false" output="|#ARGUMENTS.word#"/>
			<cfelse>
				<cfthrow detail="file not found" message="#ExpandPath('localdic/local.txt')#">
			</cfif>
		
			<cfcatch type="all">
				<cfmail to="gregory.ecklund@finalsite.com" from="cf" subject="session scope" type="html" >
	...
	<cffunction name="getMatches" access="private" returntype="array" output="false" hint="">
		...
		<cftry>
			...
			<!--- Production & test web servers are in DMZ so cfhttp doesn't work
			<cf_httpProxy url="#Local.url#" method="post" result="Local.response" timeout="5" proxyServer="#this._config.cfhttp.proxyServer#" proxyPort="#this._config.cfhttp.proxyPort#" proxyUser="#this._config.cfhttp.proxyUser#" proxyPassword="#this._config.cfhttp.proxyPassword#">
				<cf_httpProxyParam type="body" value="#Local.xml#" />
			</cf_httpProxy>
			--->
			<cfcatch>
